...
def _queue_active_ticket_predicate(settings: Settings) -> Tuple[str, List[Any]]:
    """Cola operativa: nuevo, en curso, planificado y en espera (excluye resuelto y cerrado)."""
    parts = [
        "t.is_deleted = 0",
        "t.status IN (%s, %s, %s, %s)",
    ]
    params: List[Any] = [
        settings.status_new,
        settings.status_processing,
        settings.status_planned,
        settings.status_waiting,
    ]
    if settings.entities_id is not None:
        parts.append("t.entities_id = %s")
        params.append(settings.entities_id)
...
def _project_type_ticket_filter(project_type_id: Optional[int]) -> Tuple[str, List[Any]]:
    """EXISTS clause + params to restrict ticket alias `t` to a project type.
    Returns ('', []) when project_type_id is None so callers are always safe."""
    if not project_type_id:
        return "", []
    sql = """
    AND (
        EXISTS (
            SELECT 1 FROM glpi_tickets_users _tpt
            JOIN glpi_plugin_fields_userproyectorelacionadousers _uppt ON _uppt.items_id = _tpt.users_id
            JOIN glpi_projects _prpt
                ON _prpt.id = CAST(
                    REPLACE(REPLACE(REPLACE(_uppt.projects_id_proyectorelacionadouserfield,'"',''),'[',''),']','')
                    AS UNSIGNED)
                AND _prpt.is_deleted = 0 AND _prpt.projecttypes_id = %s
            WHERE _tpt.tickets_id = t.id AND _tpt.type = 1
        )
        OR EXISTS (
            SELECT 1 FROM glpi_plugin_fields_ticketticketsformfields _pff
            JOIN glpi_projects _pp
                ON _pp.id = _pff.projects_id_proyectorelacionadofieldtwo
                AND _pp.is_deleted = 0 AND _pp.projecttypes_id = %s
            WHERE _pff.items_id = t.id
        )
    )
    """
    return sql, [int(project_type_id), int(project_type_id)]


def get_summary(settings: Optional[Settings] = None, project_type_id: Optional[int] = None) -> Dict[str, Any]:
    s = settings or get_settings()
    pred, params = _base_open_ticket_predicate(s)
    pt_sql, pt_params = _project_type_ticket_filter(project_type_id)
    ...
    row = fetch_one(
        ...
        {pt_sql}
        """,
        tuple([s.assignee_link_type] + params + pt_params),
    )
    if not row:
        return {
            "open_total": 0,
...
def get_by_assignee(settings: Optional[Settings] = None, project_type_id: Optional[int] = None) -> List[Dict[str, Any]]:
    s = settings or get_settings()
    pred, params = _base_open_ticket_predicate(s)
    pt_sql, pt_params = _project_type_ticket_filter(project_type_id)
    ...
    """
    return fetch_all(sql, tuple([s.assignee_link_type] + params + pt_params))
...
def get_by_status(settings: Optional[Settings] = None, project_type_id: Optional[int] = None) -> List[Dict[str, Any]]:
    s = settings or get_settings()
    pred, params = _base_open_ticket_predicate(s)
    pt_sql, pt_params = _project_type_ticket_filter(project_type_id)
    rows = fetch_all(
...
def get_by_priority(settings: Optional[Settings] = None, project_type_id: Optional[int] = None) -> List[Dict[str, Any]]:
    s = settings or get_settings()
    pred, params = _base_open_ticket_predicate(s)
    pt_sql, pt_params = _project_type_ticket_filter(project_type_id)
    rows = fetch_all(
...
def get_open_tickets_queue(
    limit: int = 200,
    settings: Optional[Settings] = None,
    project_type_id: Optional[int] = None,
) -> List[Dict[str, Any]]:
    """Tickets activos en cola (nuevo, en curso, en espera) con métricas de actividad.
    Usa subconsultas para proyecto y asignados para evitar filas duplicadas."""
    s = settings or get_settings()
    pred, params = _queue_active_ticket_predicate(s)
    lim = max(1, min(int(limit), 500))
    pt_sql, pt_params = _project_type_ticket_filter(project_type_id)

    udisp_a = (
        "COALESCE(NULLIF(TRIM(CONCAT(COALESCE(u_a.firstname,''),' ',COALESCE(u_a.realname,''))),''),"
        "u_a.name,CONCAT('Usuario #',u_a.id))"
    )
    assign_type = int(s.assignee_link_type)
    requester_type = int(s.requester_link_type)
    ...
    try:
        rows = fetch_all(
            ...
            SELECT
                ...
                t.name,
                t.priority,
                DATE_FORMAT(COALESCE(t.date_creation, t.date), '%%Y-%%m-%%d %%H:%%i') AS fecha_apertura,
                TIMESTAMPDIFF(HOUR, COALESCE(t.date_creation, t.date), NOW()) AS hours_open,
                (
                    SELECT COALESCE(SUM(tt2.actiontime), 0)
                    FROM glpi_tickettasks tt2
                    WHERE tt2.tickets_id = t.id
                ) AS actiontime_total,
                TIMESTAMPDIFF(HOUR,
                    (SELECT MAX(tt3.date) FROM glpi_tickettasks tt3 WHERE tt3.tickets_id = t.id),
                    NOW()
                ) AS hours_since_last_task,
                TIMESTAMPDIFF(HOUR,
                    (
                        SELECT MAX(f.date) FROM glpi_itilfollowups f
                        WHERE f.items_id = t.id AND f.itemtype = 'Ticket' AND f.is_private = 0
                          AND f.users_id IN (
                              SELECT tuc.users_id FROM glpi_tickets_users tuc
                              WHERE tuc.tickets_id = t.id AND tuc.type = {requester_type}
                          )
                    ),
                    NOW()
                ) AS hours_since_last_client_comment,
                TIMESTAMPDIFF(HOUR,
                    (
                        SELECT MAX(f.date) FROM glpi_itilfollowups f
                        WHERE f.items_id = t.id AND f.itemtype = 'Ticket' AND f.is_private = 0
                          AND f.users_id IN (
                              SELECT tua.users_id FROM glpi_tickets_users tua
                              WHERE tua.tickets_id = t.id AND tua.type = {assign_type}
                          )
                    ),
                    NOW()
                ) AS hours_since_last_assignee_comment,
                COALESCE(
                    (
                        SELECT pr2.name
                        FROM glpi_tickets_users tup2
                        JOIN glpi_plugin_fields_userproyectorelacionadousers upru2
                            ON upru2.items_id = tup2.users_id
                        JOIN glpi_projects pr2
                            ON pr2.id = CAST(
                                REPLACE(REPLACE(REPLACE(upru2.projects_id_proyectorelacionadouserfield,'"',''),'[',''),']','')
                                AS UNSIGNED) AND pr2.is_deleted = 0
                        WHERE tup2.tickets_id = t.id AND tup2.type = {requester_type}
                        LIMIT 1
                    ),
                    (
                        SELECT ppr2.name
                        FROM glpi_plugin_fields_ticketticketsformfields pltff2
                        JOIN glpi_projects ppr2
                            ON ppr2.id = pltff2.projects_id_proyectorelacionadofieldtwo AND ppr2.is_deleted = 0
                        WHERE pltff2.items_id = t.id
                        LIMIT 1
                    )
                ) AS project_name,
                (
                    SELECT GROUP_CONCAT(DISTINCT {udisp_a} ORDER BY u_a.id SEPARATOR ', ')
                    FROM glpi_tickets_users tu_a
                    JOIN glpi_users u_a ON u_a.id = tu_a.users_id AND u_a.is_deleted = 0
                    WHERE tu_a.tickets_id = t.id AND tu_a.type = {assign_type}
                ) AS assignees
            FROM glpi_tickets t
            WHERE {pred}
            {pt_sql}
        ...
        udisp_b = (
            "COALESCE(NULLIF(TRIM(CONCAT(COALESCE(u_b.firstname,''),' ',COALESCE(u_b.realname,''))),''),"
            "u_b.name,CONCAT('Usuario #',u_b.id))"
        )
        rows = fetch_all(
            f"""
            SELECT
                ...
                NULL AS hours_since_last_client_comment,
                NULL AS hours_since_last_assignee_comment,
                (
                    SELECT p.name
                ...
                (
                    SELECT GROUP_CONCAT(DISTINCT {udisp_b} ORDER BY u_b.id SEPARATOR ', ')
                    FROM glpi_tickets_users tu_b
                    JOIN glpi_users u_b ON u_b.id = tu_b.users_id AND u_b.is_deleted = 0
                    WHERE tu_b.tickets_id = t.id AND tu_b.type = {assign_type}
                ) AS assignees
            FROM glpi_tickets t
            WHERE {pred}
            ORDER BY hours_open DESC
    ...
    for r in rows:
        ...
        r["priority_label"] = PRIORITY_LABELS.get(pr, str(pr))
    return rows


def get_unassigned_samples(limit: int = 30, settings: Optional[Settings] = None) -> List[Dict[str, Any]]:
    """Wrapper de compatibilidad — delega a get_open_tickets_queue."""
    return get_open_tickets_queue(limit=limit, settings=settings)
...
def get_stale_without_followup(
    ...
    rows = fetch_all(
        ...
        LIMIT {lim}
        """,
        tuple([s.assignee_link_type] + params + pt_params + [mh]),
    )
    for r in rows:
...
def get_stale_count_vs_threshold(
    settings: Optional[Settings] = None,
    project_type_id: Optional[int] = None,
) -> Dict[str, Any]:
    s = settings or get_settings()
    ...
    row = fetch_one(
        ...
        """,
        tuple([s.stale_hours] + params + pt_params),
    )
    return {
...
def build_dashboard_payload(project_type_id: Optional[int] = None) -> Dict[str, Any]:
    s = get_settings()
    summary = get_summary(s, project_type_id=project_type_id)
    stale_info = get_stale_count_vs_threshold(s, project_type_id=project_type_id)
    return {
        "generated_for": {
            "database": s.db_name,
        ...
        "by_assignee": get_by_assignee(s, project_type_id=project_type_id),
        "management_by_assignee": get_management_by_assignee(s),
        "by_status": get_by_status(s, project_type_id=project_type_id),
        "by_priority": get_by_priority(s, project_type_id=project_type_id),
        "unassigned_top": get_open_tickets_queue(200, s, project_type_id=project_type_id),
        "stale_top": get_stale_without_followup(30, min_hours=0, settings=s, project_type_id=project_type_id),
