...
El contrato HTTP vive en `coordination_routes`.
"""
...
def coordination_weekly_assignee_performance(
    ...
    """
    Por semana ISO (mismo criterio que evolución semanal): para cada técnico que registró tareas,
    número de tickets distintos con `actiontime` en el rango y suma de segundos de esas tareas.

    Filtro temporal alineado con horas por proyecto: `(begin o date)` de la tarea intersecta
    `[date_from, date_to]` (fin de día inclusive). Alcance de ticket: entidad + tipo de proyecto
    como en el resto de coordinación (plugin Fields o `glpi_itils_projects`).

    Si `GLPI_COORD_PERFORMANCE_LOGINS` tiene valores, solo se agregan esos logins (`glpi_users.name`)
    y todas las semanas incluyen una entrada por cada uno (ceros si no hubo tiempo).
    """
    datetime.strptime(date_from, "%Y-%m-%d")
    datetime.strptime(date_to, "%Y-%m-%d")
    s = settings or get_settings()
        ...
        weeks_out.append(
            {
                ...
                "week_period_start": str(w.get("week_period_start") or ""),
                "week_period_end": str(w.get("week_period_end") or ""),
                "assignees": _coord_week_assignees_merged_for_series(ps, series_meta, by_week),
            }
        )
